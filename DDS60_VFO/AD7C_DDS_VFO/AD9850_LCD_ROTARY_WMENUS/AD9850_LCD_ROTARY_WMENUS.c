...
int_fast32_t timepassed = millis(); // int to hold the millis since startup

// value to notify if memory is current or old. 0=old, 1=current.
int memstatus = 1;

// Change this to 0 after you upload and run a working sketch to activate the
// EEPROM memory.  YOU MUST PUT THIS BACK TO 0 AND UPLOAD THE SKETCH AGAIN
// AFTER STARTING FREQUENCY IS SET!
int ForceFreq = 1;
...
void setup()
{
  ...
  pulseHigh(RESET);
  pulseHigh(W_CLK);
  pulseHigh(FQ_UD);  // enables serial mode on the AD9850 - Datasheet page 12.
  lcd.setCursor(hertzPosition,1);
  lcd.print(hertz);
  // Load the stored frequency
  if (ForceFreq == 0)
  {
    freq = String(EEPROM.read(0)) + String(EEPROM.read(1)) +
           String(EEPROM.read(2)) + String(EEPROM.read(3)) +
           String(EEPROM.read(4)) + String(EEPROM.read(5)) +
...
void loop()
{
  ...
  // Write the frequency to memory if not stored and
  // 2 seconds have passed since the last frequency change.
  if (memstatus == 0)
  {
...
ISR(PCINT2_vect)
{
  ...
  if (result)
  {
    ...
    if (rx >=30000000)    // UPPER VFO LIMIT
    {
      rx=rx2;
    }
    if (rx <=1000000)     // LOWER VFO LIMIT
    {
      rx=rx2;
    }
  }
...
void sendFrequency(double frequency)
{
  ...
  // Make 'slight' tuning variations here by adjusting the clock frequency.
...
void setincrement()
{
  ...
  lcd.setCursor(hertzPosition,1);
  lcd.print(hertz);
  delay(250); // Adjust to speed up/slow down the button menu scroll speed.
}
