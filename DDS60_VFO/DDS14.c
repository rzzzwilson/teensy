...
// our main timing variable instead of using millis() directly.  See main loop
int _ms;

// used by interupt code to communicate encoder rotation to main loop
volatile byte _encoderCount  = 0; 

// encoder speed setting. 1 = maximum speed, 2 = half speed etc.
// This should only be set to a power of 2.
...
// put the memory channels at the top of the EEPROM
const int EE_MEMORY_CHANNELS = 1024 - (MEMORY_CHANNEL_MAX * 4);

// frequency range
const long FREQ_MIN = 1000000;    // 1 MHz
const long FREQ_MAX = 60000000;   // 60 MHz

// the limits of the LSB of the clock value. 180,000,000 is ABA9500 hex.
// We make use of the convenient fact that + or - 100 ppm (18 KHz) only
// changes the 2 least significant bytes. This means that we can do a simple
// and quick OR with CLOCK_BASE to get the final clock to calculate the delta
// to send to the DDS
...
// Upper bits of 180 MHz. ORed with _clockLSB when programming DDS
...
// calibration value. 0x9500 for clock = exactly 180 MHz
unsigned int _clockLSB;

// save _clockLSB in case we don't commit to calibration adjustment
unsigned int _saveClockLSB;
...
// set of the frequency has changed. Used for saving frequency to EEPROM
boolean _frequencyHasChanged = false;
...
// CW audio frequency in Hz.  This controls both the DDS frequency shift
// between TX and RX and the sidetone audio.
const int CW_FREQ = 800;

// transmit timeout in ms.
// This is the key up time required before switching back to receive
...
void setup()
{
  ...
  // Set push button port numbers. The array index starts at zero but I find it
  // easier to think of the buttons as 1, 2 and 3.
  // We use the Arduino digital port functions for the (slow) buttons so it's
  // easy to add more buttons to the 3 spare ports but, to get good performance,
  // we write directly to the port registers for the DDS, LCD and encoder.
  ...
  // CW port is an input.  Turn on the pull-up resistor for the open collector
  // interface on the transeiver.
  digitalWrite(CW_PORT, HIGH);
  ...
  // if it is outside + or - 9999 Hz then set it from 0 Hz,
  // i.e., clock assumed to be exactly  180 MHz
  ...
  // write 0 to the DDS. This might not be necessary but it keeps it off
  // until we set it to a known frequency
  WriteToDDS(0);

  // enable interupts and attach our interupt routines.
  // Set to trigger on every transition of the inputs.
  ...
  // set it to some favorite frequency if it is out of range.
  // This should really only happen on the first power-up.
  ...
  // Saving the mode is mostly for the benefit of QRSS mode so we restart after
  // a power failure. There doesn't seem much sense to start in CALIBRATE or
  // MEMORY mode so we start in TRANS, QRSS or NORMAL.
...
void loop()
{
    // We need to keep track of elapsed time for the various delays. millis()
    // returns a 32 bit long.  If we only use the lower 16 bits then it will
    // cycle around (overflow) every 65 seconds or so. That's plenty for us.
    // Since _ms is an int, our other time variables only need to be ints
    // rather than longs. With the usual 2s complement arithmetic, "elapsed"
    // calculations work fine without us needing to do anything special
    // for overflow.
    ...
    // Because of the save delay to protect the EEPROM from excessive writes,
    // this is called every time whether or not an event happened.
...
void DoCW()
{
  ...
  if (_key_down)
  {
    if (!_transmit)
    {
      // if key is down and we are not on transmit then go to transmit 
      SetTxRx(true);
    }
    
    if (!was_key_down)
    {
      // if key is down but was up before (just pressed) then start sidetone
  ...
  else
  {
    ...
    if (was_key_down)
    {
      // if key up but was down before (just been released) then end sidetone
      noTone(SIDETONE_PORT);
    }
    
    if (_transmit)
    {
      // if we're on transmit but the key has been up for the timeout
      // period then go to receive.
...
// -----------------------------------------------------------------------------
// NORMAL mode.
// This is the normal "turn the dial and the frequency changes" mode
// -----------------------------------------------------------------------------
...
void NormalMode()
{
  switch(_event)
  {
    case ENCODER_TURN_EVENT:  // encoder was turned
      // adjust the frequency by the amount the encoder moved
      _frequency += ((long) _eventParam) * _frequencyStep;
      // restrict the frequency to the allowable range
      _frequency = RestrictFrequency(_frequency);
      // set the DDS to the new frequency
      SetDDSFrequency();
      // print the new frequency
      PrintFrequency(_frequency);
      // remember that frequency has changed so we can save it to EEPROM later
    ...
    case ENCODER_TURN_EVENT:	// encoder was turned
      if (!_transmit)		// ignored on transmit
      {
        ...
        else
        {
          ...
          // print the new frequency
          PrintFrequency(_frequency);
         // remember that frequency has changed so can save it to EEPROM later
          _frequencyHasChanged = true;
        }
...
void SetRit(boolean onOff)
{
  ...
  {
    // logic so we can call this even if RIT was already off
    if (_mode == TRANS_MODE && _ritOn)
    {
...
void MemoryMode()
{
  switch(_event)
  {
    ...
    case BUTTON_PUSH_EVENT:    // a button was pressed
      ...
      {
        case 1:
          // move to calibrate mode
          SetMode(CALIBRATE_MODE);
          break;
          
        case 2:
          // if the selected channel is not "empty" then set the DDS to the
          // new frequency and return to NORMAL mode
          if (_memoryFrequency)      
          {
        ...
        case 3:
          // we have chosen to save into this channel so move to confirm mode
          SetMode(MEMORY_CONFIRM_MODE);
          break;
      }
  }
}
...
{
  // the display is showing "Save ?" and waiting for a button push
...
void CalibrateMode()
{
  ...
  {
    case ENCODER_TURN_EVENT:  // the encoder was turned
      // move the _clockLSB calibration value up or down
      _clockLSB += ((long) _eventParam) * _frequencyStep;
      RestrictClock();    // restrict to + or - 9999
      PrintClock();       // print the new calibration value
      SetDDSFrequency();  // set DDS frequency using the new calibration value
      break;
    ...
    case BUTTON_PUSH_EVENT:
      ...
      {
        case 1:
          // Return to normal mode without saving calibration value
...
void SetMode(byte mode, boolean init)
{
  ...
  switch(mode)
  {
    ...
    case MEMORY_MODE:
     // set frequency in case we were shifted for QRSS key down
      SetDDSFrequency();
      _encoderSpeed = 2;          
      PrintMode((char *) " Mem ");
      ClearLowerRight();
...
{
  char str[5];
  sprintf(str, "M %2i", _memoryChannel);  // 'M nn' where nn is memory channel
  LCDPrint(str, 0, 1);

  _memoryFrequency = ReadLongFromEEPROM(EE_MEMORY_CHANNELS +
                         ((_memoryChannel - 1) * 4));
...
void SaveStateToEEPROM()
{
  ...
  // Note: a static variables retains its value on each call of this routine
  static int lastSaveMS;
...
void ReadButtons()
{
  ...
  const byte WAIT_FOR_DOWN = 0;
  // wait for button to be released or for it to be pressed long enough
  // to be considered "held".
  const byte WAIT_FOR_UP_OR_TIME = 1;
  ...
  // We only want to know about one button down at the time.  i.e, we're not
  // implementing any "shift" keys. Note that the other side of the buttons are
  // grounded so the port reads zero when a button is pressed.
  
  // My buttons are all on PORTB so the code could be simplified and made
  // slightly faster by reading PORTB directly in one operation but this is more
  // generic and makes it simple to change the port assignments
  // or add more buttons.
  ...
  switch(state)
  {
    case WAIT_FOR_DOWN:
      if (buttonDown)  // a button is pressed
      {
       ...
       // remember which button is active for next time around the loop
        activeButton = buttonDown;

        _event = BUTTON_PUSH_EVENT;    // raise button push event
        _eventParam = activeButton;    // tell other code which button pressed
      }
      break;
    ...
    case WAIT_FOR_UP_OR_TIME:
      if (buttonDown != activeButton)
      {
        ...
        if (buttonDown)
        {
        // we prefer all buttons to be up now. i.e, we're do cope with
        // two buttons pressed at the same time
          state = WAIT_FOR_ALL_UP;
        }
        else
        {
          // nothing pressed now so go back to waiting for the next button press
          state = WAIT_FOR_DOWN;
        }
      }
      else
      {
        ...
        if (elapsed >= 5000)      // "held" if it has been pressed for 5 seconds
        {
...
void ReadEncoder()
{
  ...
  // read the count into another variable. It's a single byte so it's safe to
  // read without stopping interupts
  ...
  if (diff != 0)
  {
    ...
    prevCount = count;              // remember for next time
     ...
     _eventParam = diff;            // tell other code how far we moved
...
// by knowing which pin caused the interupt and the state of both pins,
// we can figure out which direction it moved
// -----------------------------------------------------------------------------
...
void PrintFrequency(long frequency)
{
  ...
  {
    // this formats the frequency in the typical transceiver format with
    // two decimal points separating MHz and KHz
    char str[11];
...
{
  // get the clock frequency to use.
  // if _clockLSB is 0x9500 then this is exactly 180 MHz
  long clock = CLOCK_BASE | _clockLSB;
...
void WriteByteToDDS(byte data)
{
  ...
  for (int i = 0; i < 8; i++)
  {
      ...
      // 1us delays are much longer than we need. The AD9851 datasheet indicates
      // that only a few ns are necessary but it seems foolish to push our luck
      // with long wires to the display etc.  with 1us delays we still shift out
      // 40 bits in about 130us which is plenty fast enough and much faster than
      // using ShiftOut which takes about 620us.
...
void SaveToEEPROM(unsigned int value, int address)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    // only write if we really need to so we avoid excessive writes
...
// clear an area of the display. This is probably more efficient and wastes less
// space than potentially having "   " of varying lengths throughout the code.
...
void QRSSMode(boolean reset)
{
    ...
    // put your message here. Usually this is just your callsign. You can only
    // use characters in the table above.
    static char message[] = "YOUR MESSAGE";

    // change fSKShift and dotime to something like 500 and 100 if you want
    // to hear normal speed CW for testing
    ...
    if (_event == BUTTON_PUSH_EVENT)
    {
      ...
      {
        case 1:
          // button 1 so move to memory mode
          SetMode(MEMORY_MODE);
          break;

         case 2:
          // button 2 sets us back at the beginning of the message
          state = RESET;
          break;
      }
      _event = 0;
    ...
    char toSend;
    int elapsed;                   // how long since we were here last time
    
    // what state are we in?
    // We ignore the 2 most significant bits so the KEY_UP_WAITs go together
    switch (state & 0x3F)
    {
      case RESET:
      ...
      case START_CHARACTER:
        ...
        if (toSend == ' ')
        {
          // if it's a space then we wait 4 dot times to give a total of 7 dot
          // times because we have already done 3, i.e, 1 at the end of the last
          // element and 2 at the end of the last character
          dotsToWait = 4;
          lastMS = _ms;
        ...
        else
        {
          ...
          {
            // the 5 is because of a gap in our table between ASCII '9' and '?'
            cwBits = cwCodes[toSend - '.' - 5];
          }
      ...
      case START_ELEMENT:
        ...
        // key stays down for 1 dot time for a dit, 3 for a dah
        dotsToWait = cwBits & bitMask ? 3 : 1;
        ...
        if (elapsed > dotTime)     // have we done a dot time yet?
        {
          ...
          if (!dotsToWait)        // have we waited all the required dot times?
          {
            ...
            else
            {
              switch(state)
              {
                case KEY_UP_END_OF_ELEMENT:  // we just finished the 1 dot wait
                  ...
                  {
                     // we're finished this character so wait 2 more dot times
                     // to give a 3 dot character spacing
                     dotsToWait = 2;
                     state = KEY_UP_END_OF_CHARACTER;
                  }
                  break;
                  
                case KEY_UP_END_OF_CHARACTER:
                  // we just finished the 3 (1+2) dot wait after a character
                  charIdx++;                   // point to next char in the msg
                  if (message[charIdx])        // is this the end of the msg
                  {
                    state = START_CHARACTER;   // not yet so send next character
                  }
                  else
                  {
                    // end of msg to wait 4 dot time. this gives a total of 7
                    // dot times so we're effectively sending a space before
                    // starting the message again.
                    // you might want to make that time a little bigger.
                    dotsToWait = 4;
                    state = KEY_UP_END_OF_STRING;
                  }
                  break;
                  
                case KEY_UP_END_OF_STRING:
                  // we just finished the space at the end of the message
                  charIdx = 0;     // reset pointer and start the message again
